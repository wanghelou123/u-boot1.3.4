...
static int check_command_end(ushort cmd, uint resp)
{
	...
	if(!(resp&WAIT_RESP)) {
		DBG("!(sdi_csta & CMD_END)\n");
		while(!(sdi_csta & CMD_END))
			sdi_csta = readl(SDICSTA);

		DBG("iwritel(sdi_csta, SDICSTA);\n");
		writel(sdi_csta, SDICSTA);
		return 0;
	}
	else {
		DBG("!((sdi_csta&RESP_END) || (sdi_csta&CMD_TOUT)).\n");
		while(!((sdi_csta&RESP_END) || (sdi_csta&CMD_TOUT)))
			sdi_csta = readl(SDICSTA);

		DBG("writel(sdi_csta, SDICSTA).\n");
		writel(sdi_csta, SDICSTA);
		return 0;
	}
	return 1;
...
static int mmc_setting(void)
{
	...
	DBG("readl(SDIFSTA)\n");
	regs = readl(SDIFSTA);
	DBG("writel(regs | FIFO_RESET, SDIFSTA)\n");
	writel(regs | FIFO_RESET, SDIFSTA);

	DBG("readl(SDIBSIZE) & ~0xfff\n");
	regs = readl(SDIBSIZE) & ~0xfff;
	DBG("writel(regs | MMC_BLOCK_SIZE, SDIBSIZE)\n");
	writel(regs | MMC_BLOCK_SIZE, SDIBSIZE);

	DBG("mmc_cmd(MMC_GO_IDLE_STATE, 0,  SHORT_RESP | WAIT_RESP)\n");
