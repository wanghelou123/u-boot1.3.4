...
#undef DEBUG_HSMMC
//#define  DEBUG_HSMMC  /*modified by whl 2015-08-03 */
#ifdef DEBUG_HSMMC
...
enum card_type card_type;/*add by whl 2015-08-04 */
...
static int wait_for_cmd_done (void)
{
	...
	while(1){
		...
		if (n_int & 0x8000) 
			break;	// 数据手册492页，最高位为1说明有错误发生
		if (n_int & 0x0001) 
			return 0;//最低位为1说明 写缓冲区准备好了
	}

	e_int = s3c_hsmmc_readw(HM_ERRINTSTS);
	s3c_hsmmc_writew(e_int, HM_ERRINTSTS);
	s3c_hsmmc_writew(n_int, HM_NORINTSTS);	//e_int=0x0001==>说明超时
...
#if 0	/*modfied by whl 2015-08-04 */
	if (mmc_card) {
		if (mmc_spec == 4) {
...
//	writel(reg | (1<<27), SCLK_GATE);
	...
	if (!issue_command(MMC_ALL_SEND_CID, 0, 0, MMC_RSP_R2)) { /* cmd2 */
		puts("CID broken\n");
		return -3;
	}
...
void movi_read (uint addr, uint start_blk, uint blknum)
{
	//printf("addr = %d, start_blk = %d, blknum=%d\n", addr, start_blk, blknum);
	uint dma=0,cmd,multi_blk,i;
	...
	for(i=0;i<blknum;i++){
		...
		SetSystemAddressReg(addr);                        //AHB System Address For Write
		addr += Card_OneBlockSize_ver1;

		set_blksize_register(0,Card_OneBlockSize_ver1);   //Maximum DMA Buffer Size, Block Size
		set_blkcnt_register(1);                           //Block Numbers to Write
