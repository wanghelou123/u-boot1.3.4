...
int do_movi(cmd_tbl_t * cmdtp, int flag, int argc, char *argv[])
{
	...
	if (strcmp(cmd, "init") == 0) {
		...
		/*
			printf("u-boot:%x - %x.\n",ofsinfo.bl2, MOVI_BL2_BLKCNT);
			printf("kernel:%x - %x.\n", ofsinfo.kernel, MOVI_ZIMAGE_BLKCNT);
			printf("rootfs:%x - %x.\n", ofsinfo.rootfs, MOVI_ROOTFS_BLKCNT);
			printf("ofsinfo.bl1:%d\n", ofsinfo.bl1);
			printf("ofsinfo.last:%d\n", ofsinfo.last);
			printf("ofsinfo.env:%d\n", ofsinfo.env);
		*/
