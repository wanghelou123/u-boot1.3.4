...
{
	if (On) {
		rHM_CLKCON |= (1<<2); // sd时钟使能
		while (!(rHM_CLKCON & (1<<3))) {
			// 等待SD输出时钟稳定
		}
	} else {
		rHM_CLKCON &= ~(1<<2); // sd时钟禁止
	}
}
...
static int Hsmmc_WaitForCommandDone()
{
	...
	for (i=0; i<20000000; i++) {
		if (rHM_NORINTSTS & (1<<15)) { // 出现错误
			break;
		}
		if (rHM_NORINTSTS & (1<<0)) {
			do {
				rHM_NORINTSTS = (1<<0); // 清除命令完成位
			} while (rHM_NORINTSTS & (1<<0));			
			return 0; // 命令发送成功
		}
	}
	ErrorState = rHM_ERRINTSTS & 0x1ff; // 可能通信错误,CRC检验错误,超时等
	rHM_NORINTSTS = rHM_NORINTSTS; // 清除中断标志
	rHM_ERRINTSTS = rHM_ERRINTSTS; // 清除错误中断标志	
	do {
		rHM_NORINTSTS = (1<<0); // 清除命令完成位
	} while (rHM_NORINTSTS & (1<<0));
...
static int Hsmmc_WaitForTransferDone()
{
	...
	for (i=0; i<20000000; i++) {
		...
		if (rHM_NORINTSTS & (1<<1)) { // 数据传输完								
			do {
				rHM_NORINTSTS |= (1<<1); // 清除传输完成位
			} while (rHM_NORINTSTS & (1<<1));	
			rHM_NORINTSTS = (1<<3); // 清除DMA中断标志								
			return 0;
		}
		Delay_us(1);
	}
...
int Hsmmc_GetSdState(unsigned char *pState)
{
	...
	if (CardType == SD_HC || CardType == SD_V2 || CardType == SD_V1) {
		...
		rHM_SYSAD = (unsigned int)Hsmmc_Buffer; // 缓存地址	
		rHM_BLKSIZE = (7<<12) | (64<<0); // 最大DMA缓存大小,block为512位64字节			
		rHM_BLKCNT = 1; // 写入这次读1 block的sd状态数据
		rHM_ARGUMENT = 0; // 写入命令参数	
...
int Hsmmc_GetCardState(void)
{
	if (Hsmmc_IssueCommand(CMD13, RCA<<16, 0, Response_R1)) {
		...
		return ((rHM_RSPREG0>>9) & 0xf); // 返回R1回复中的[12:9]卡状态
	}
}
...
int Hsmmc_ReadBlock(unsigned char *pBuffer, unsigned int BlockAddr, unsigned int BlockNumber)
{
	...
	// 均不中断使能,产生相应的中断信号
	rHM_NORINTSIGEN &= ~0xffff; // 清除所有中断使能
	rHM_NORINTSIGEN |= (1<<1); // 命令完成中断使能
	...
	while (BlockNumber > 0) {
		...
		rHM_SYSAD = (unsigned int)Hsmmc_Buffer; // 缓存地址,内存区域为关闭cache,作DMA传输			
		rHM_BLKSIZE = (7<<12) | (512<<0); // 最大DMA缓存大小,block为512字节			
		rHM_BLKCNT = ReadBlock; // 写入这次读block数目
...
int Hsmmc_WriteBlock(unsigned char *pBuffer, unsigned int BlockAddr, unsigned int BlockNumber)
{
	...
	while (BlockNumber > 0) {
		...
		rHM_SYSAD = (unsigned int)Hsmmc_Buffer; // 缓存地址,内存区域为关闭cache,作DMA传输		
		rHM_BLKSIZE = (7<<12) | (512<<0); // 最大DMA缓存大小,block为512字节		
		rHM_BLKCNT = WriteBlock; // 写入block数目	
...
int do_hsmmc_init()
{
	...
	// 设置HSMMC1的接口引脚配置,数据手册260页
	Debug("config GPLCONF register.\n");
	rGPLCON &= ~((0xffff<<0) | (0xf<<16));
	rGPLCON |= (0xaaaa<<0) | (0xa<<16);
	rGPLUDP &= ~((0xffff<<0) | (0xf<<16)); // 上下拉禁止

	//设置HSMMC0的接口引脚配置，数据手册251页
	//Debug("config GPECONF register.\n");
	//rGPECON &= ~(0xFFF<<10);
	///rGPECON |=  0xAAA<<10;	//配置为功能引脚
	//rGPEUP &= ~(0xFFF<10);	//禁止上下拉

	Debug("reset HSMMC.\n");
	//rHM_SWRST = 0x7; // 复位HSMMC
	Debug("Hsmmc_SetClock(0x80).\n");
	Hsmmc_SetClock(0x80); // SDCLK=96M/256=375K
	Debug("set timeout.\n");
	rHM_TIMEOUTCON = (0xe << 0); // 最大超时时间
	Debug("set normal speed.\n");
	rHM_HOSTCTL &= ~(1<<2); // 正常速度模式
	Debug("clear irq.\n");
	rHM_NORINTSTS = rHM_NORINTSTS; // 清除中断状态标志
	rHM_ERRINTSTS = rHM_ERRINTSTS; // 清除错误中断状态标志
	Debug("enable irq.\n");
	rHM_NORINTSTSEN = 0x7fff; // [14:0]中断使能
	rHM_ERRINTSTSEN = 0x3ff; // [9:0]错误中断使能
	...
	if (Hsmmc_IssueCommand(CMD8, 0x1aa, 0, Response_R7)) { // 没有回复,MMC/sd v1.x/not card
		for (i=0; i<1000; i++) {
			...
			if (!Hsmmc_IssueCommand(CMD41, 0, 0, Response_R3)) { // CMD41有回复说明为sd卡
				OCR = rHM_RSPREG0; // 获得回复的OCR(操作条件寄存器)值
		...
		Debug("ok2\n");
		if (((rHM_RSPREG0&0xff) == 0xaa) && (((rHM_RSPREG0>>8)&0xf) == 0x1)) { // 判断卡是否支持2.7~3.3v电压
			OCR = 0;
			for (i=0; i<1000; i++) {
				Hsmmc_IssueCommand(CMD55, 0, 0, Response_R1);
	...
	if (CardType == SD_HC || CardType == SD_V1 || CardType == SD_V2) {
		...
		RCA = (rHM_RSPREG0 >> 16) & 0xffff; // 从卡回复中得到卡相对地址
		...
		if (Hsmmc_GetCardState() == 4) { // 此时卡应在transfer态
			if (!Hsmmc_IssueCommand(CMD16, 512, 0, Response_R1)) { // 设置块长度为512字节
				rHM_NORINTSTS = 0xffff; // 清除中断标志
